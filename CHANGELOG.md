...
### Added
- **On-demand status refresh**: `MirAIeAPI.refresh_status(device)` fetches the latest status from the REST API and updates `device.status` in place, instead of waiting for the next MQTT push
- **AC controller daemon (example)**: `example/ac-daemon.py` is an always-on service that keeps a running AC in a desired configuration, re-checking every N seconds (default 300) and correcting only drifted settings. It never powers the AC on by itself during the periodic loop, so turning it off with the remote is respected; power-on happens only on an explicit Start request. Start/stop/status via a token-protected HTTP API (intended for Tailscale), plus a built-in mobile-friendly web UI at `/` with live status, a Refresh button (forces a cloud reading via `/refresh`), and log output. Ships with `ac_daemon.ini.example` and a `ac-daemon.service` systemd unit
