...
### Added
- **AC controller daemon (example)**: `example/ac-daemon.py` is an always-on service that maintains a desired AC configuration and re-applies it every N seconds (default 300) to recover from power cuts. Start/stop/status via a token-protected HTTP API (intended for Tailscale), plus a built-in mobile-friendly web UI at `/` with live status and log output. Ships with `ac_daemon.ini.example` and a `ac-daemon.service` systemd unit
